CREATE TABLE `adv_project`.`monitor_advertisements` (
  ...
  `user_id` BIGINT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_monitor_advertisements_types_idx` (`type_id` ASC) VISIBLE,
  INDEX `fk_monitor_advertisements_city_idx` (`city_id` ASC) VISIBLE,
  INDEX `fk_monitor_advertisement_sub_categories_idx` (`sub_category_id` ASC) VISIBLE,
  INDEX `fk_monitor_advertisement_users_idx` (`user_id` ASC) VISIBLE,
